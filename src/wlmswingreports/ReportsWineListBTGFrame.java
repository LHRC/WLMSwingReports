 ...
 */
public class ReportsWineListBTGFrame extends JInternalFrame
{

    /** Creates new form ReportsInventoryExtensionFrame */
    public ReportsWineListBTGFrame()
    {
      //super(mit);
      try
      {
  ...
  private void getAllocations() throws SQLException
  {
    ...
    //ps.setDate(1, new SimpleDate().toSQLDate());
    ResultSet rs = ps.executeQuery();
    while (rs.next())
    {
...
//    JasperCompileManager.compileReportToFile(reportSource, compiledReport);
      ...
      InputStream jasper1 = getClass().getResourceAsStream("WineListBTGReport.jasper");
  ...
  private void getData() throws SQLException
  {
     String q = "select wlc.wine_list_category_name, pi.wine_list_name, " +
              "pi.wine_list_description, pi.sale_price, pi.glass_sale_price, " +
              ...
              "where pi.is_by_the_glass and " +
              "pi.wine_list_bin_number is not null " +
...
//          name.replace(",”", "”");
      ...
      line.add(scName);
      line.add( Integer.parseInt(ndp.format(rs.getFloat("glass_sale_price"))));
  ...
  DecimalFormat ndp = new DecimalFormat("#,##0");
  String reportPath = "lib/reports/";
  String compiledReport = reportPath + "WineListBTGReport.jasper";
  String reportSource = reportPath + "WineListBTGReport.jrxml";
