 ...
 */
public class ReportsWineListNewProductsFrame extends JInternalFrame
{

    /** Creates new form ReportsInventoryExtensionFrame */
    public ReportsWineListNewProductsFrame()
    {
      try
      {
        tcm = new TheoreticalCountMap();
        getDateRange();
        getAllocations();
        getSubcategories();
        getReportData();
  ...
  void getDateRange()
  {
    String s = JOptionPane.showInputDialog("Number of Days", "7");
    Integer i = Integer.parseInt(s);
    numberOfDays = ( i == null) ? 7 : i;
  }
  ...
  private void getAllocations() throws SQLException
  {
    String q = "select a.product_instance_id, sum(a.amount) as total " +
               ...
               "where a.quarter_id = q.quarter_id " +
               "and q.start_date > ? " +
  ...
  private void getData() throws SQLException
  {
     String q = "select wlc.wine_list_category_name, pi.wine_list_name, " +
              "pi.wine_list_description, pi.sale_price, pi.pos_story, " +
              ...
              "and pi.product_instance_id in " +
              "(select pila.product_instance_id from invoices i, " +
              "invoice_details id, product_instance_location_associations pila " +
              "where i.invoice_id = id.invoice_id and " +
              "pila.product_instance_location_id = id.product_instance_location_id " +
              "group by pila.product_instance_id " +
              "having min(i.payment_date) >= ?) " +
...
//          name.replace(",”", "”");
      ...
      line.add(scName);
      String story = rs.getString("pos_story");
      if (rs.wasNull())
      {
        story = null;
      }
  ...
  String compiledReport = "WineListNewProductsReport.jasper";
